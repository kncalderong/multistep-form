...
const FormMultipleOptions = ({ isIncluded, name, description, price, value, setFunction }: FormMultipleOptionsProps) => {
  return (
    <div className={`flex px-4 pt-[14px] pb-[18px] justify-start items-center ${isIncluded ? 'border-purplish-blue bg-very-light-gray' : 'border-light-gray bg-white'} rounded-lg border-[1px] gap-[16px] cursor-pointer md:hover:border-purplish-blue`} onClick={() => setFunction(value)}>
      ...
      <div className='flex justify-between grow'>
        <div className='flex flex-col gap-[3px]' >
          <p className='text-marine-blue font-medium text-sm md:text-base md:font-bold' >{name}</p>
          <p className='text-cool-gray text-xs md:text-sm' >{description}</p>
        </div>
        <p className='text-purplish-blue text-xs flex justify-center items-center md:text-sm' >{price}</p>
      </div>
    </div>
  )
}
