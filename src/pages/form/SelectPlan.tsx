...
const SelectPlan = () => {
  ...
  const { selectedPlan, isMothlyPlan, updateInfo } = useAppContext()
  const [selectedPlanLocal, setSelectedPlanLocal] = useState<planOptions >(selectedPlan)
  const [isMonthly, setIsMonthly] = useState<boolean>(isMothlyPlan)
        ...
        <div className='bg-white py-8 px-6 w-full rounded-[10px] mt-[-89px] ' >
          ...
          <h2 className='text-2xl text-marine-blue font-bold mb-2' >Select your plan</h2>
            ...
            <div className='flex h-12 rounded-lg bg-very-light-gray py-[14px] gap-6 justify-center items-center' >
              ...
              <div className={`w-[38px] h-[20px] rounded-[10px] bg-marine-blue flex items-center ${isMonthly ? 'justify-start' : 'justify-end'} p-1 cursor-pointer`} onClick={()=> setIsMonthly(!isMonthly) } >
